...
def vote_with_token(s: Session, token: str, vote: str, poll_id: str):
...
def count_polls(s: Session) -> int:
    return s.query(Poll).count()

def create_poll(s: Session, order: int, description: str) -> Poll:
...
def get_votes_for(s: Session, db: SQLAlchemy, poll_id: str):
    return db.Query([
            db.func.count(db.case(
                [((Vote.vote_option == 0), Vote.user_token)],
                else_=db.literal_column("NULL")
